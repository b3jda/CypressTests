describe('Transaction History Page', () => {
  beforeEach(() => {
      cy.visit('http://127.0.0.1:8000/transactionhistory.php');
  });

  it('should load the transaction history page correctly', () => {
      cy.get('title').should('contain', 'Transaction History');
      cy.get('h2').should('contain', 'Transaction History');
  ...
  it('should have the correct table structure', () => {
      cy.get('table').within(() => {
          ...
          cy.get('th').eq(0).should('contain', 'S.No.');
          cy.get('th').eq(1).should('contain', 'Sender');
          cy.get('th').eq(2).should('contain', 'Receiver');
          cy.get('th').eq(3).should('contain', 'Amount');
          cy.get('th').eq(4).should('contain', 'Date & Time');
      });
  });

  it('should display transaction information in the table', () => {
  ...
  it('should correctly display the amount with currency format', () => {
      cy.get('table tbody tr').each(($row) => {
          cy.wrap($row).within(() => {
              cy.get('td').eq(3).invoke('text').should('match', /Rs\.\s\d+\s\/-/);
          });
      });
  });
});
